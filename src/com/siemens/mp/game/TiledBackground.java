...
public class TiledBackground extends GraphicObject 
{
	...
	public TiledBackground(Image tilePixels, Image tileMask, byte[] map, int widthInTiles, int heightInTiles) 
    {
		...
		if (tileMask != null) { tilePixels = com.siemens.mp.lcdui.Image.createTransparentImageFromMask(tilePixels, tileMask); }
	...
	protected void paint(Graphics g, int x, int y) 
    {
        y += posy;
        x += posx;
		for (int i = y / 8; i < heightInTiles; i++) 
        {
			for (int j = x / 8; j < widthInTiles; j++) 
            {
				g.drawImage(pixels[map[i * widthInTiles + j] & 0xFF], -posx + j * 8, -posy + i * 8, 0);
			}
		}
	}
}